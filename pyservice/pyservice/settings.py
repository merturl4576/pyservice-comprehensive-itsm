...
# CSRF & Security
CSRF_TRUSTED_ORIGINS = config('CSRF_TRUSTED_ORIGINS', default='http://localhost,http://127.0.0.1,https://*.onrender.com', cast=Csv())
...
if DATABASE_URL:
    ...
    DATABASES = {
        'default': {
            'ENGINE': DB_ENGINE,
            'NAME': BASE_DIR / 'pyservice_db.sqlite3' if 'sqlite' in DB_ENGINE else config('DB_NAME', default='pyservice_db'),
...
if REDIS_URL:
    ...
    # Fallback: in-memory channel layer (works for single-instance deployments)
    CHANNEL_LAYERS = {
        'default': {
...
AUTHENTICATION_BACKENDS = [
    'pyservice.auth_backend.CompanyAuthBackend',
